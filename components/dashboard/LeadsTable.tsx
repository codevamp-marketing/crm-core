...
import { Search, Filter, Plus, MoreHorizontal, Eye, Clock, X, ChevronUp, ChevronDown, ArrowUpDown, Loader2, Send } from 'lucide-react';
...
export default function LeadsTable({ mode, title, subtitle, isPicked, pickedBy }: LeadsTableProps) {
    ...
    // Fetches real activity data + subscribes to Supabase Realtime for new rows.
    const { activities, isActivitiesLoading } = useLeadDetail(selectedLead?.id ?? null);
    const createActivity = useCreateActivity();
    useActivitiesRealtime(selectedLead?.id ?? null);
    ...
    const handleAddNote = () => {
        ...
        createActivity.mutate(
            { leadId: selectedLead.id, type: 'Note', description: note.trim() },
            { onSuccess: () => setNote('') },
        );
    };
                    ...
                    <div className="flex justify-between items-end">
                        <div>
                            <div className="flex items-center gap-3">
                                ...
                                {mode === 'fresh' && (
                                    <span className="px-2.5 py-1 bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300 text-xs font-bold uppercase tracking-wider rounded-lg border border-amber-200 dark:border-amber-800">
                                        Unassigned
                                    </span>
                                )}
                                {mode === 'my_leads' && (
                                    <span className="px-2.5 py-1 bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300 text-xs font-bold uppercase tracking-wider rounded-lg border border-emerald-200 dark:border-emerald-800">
                                        Assigned to You
                                    </span>
                                )}
                            </div>
                        ...
                        <Table>
                            <TableHeader className="bg-zinc-50/50 dark:bg-zinc-900/50">
                                <TableRow className="border-[var(--border-subtle)]">
                                    ...
                                    <TableHead className="font-semibold text-[var(--text-secondary)] text-xs uppercase tracking-wider text-center w-[160px]">Activity</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
